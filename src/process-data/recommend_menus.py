...
def craft_day_level_menu(
    meals_by_day: dict, restaurant: int, weekday: int, n_max: int = 10_000_000
) -> list:
    ...
    combos = [
        [x[0], *x[1]] for x in product(non_vegan, combinations(vegan, NUM_VEGAN_PER_DAY))
    ]
...
def main():
    ...
    for date_firstweek in pd.date_range(date_start, date_end, freq="7d"):
        logger.info(
            f"Craft menu for restaurant: {restaurant} - week: {date_firstweek.date()}"
        )
        ...
        # =================================================
        meal_type_fish = (
            dim_meal_types.filter(pl.col("meal_type_en") == pl.lit("fish"))[
                "meal_type_id"
            ]
            .head()
            .item()
        ...
        menus_week = pl.DataFrame(
            {
                ...
                "weeklevel_idx": pl.Series(
                    [str(uuid.uuid4()) for _ in range(NUM_DAY_LEVEL_MENUS)]
                ),
            }
        )
        ...
        weekday2date = pl.DataFrame(
            {
                "weekday": [1, 2, 3, 4, 5],
                "date": pl.Series(pd.date_range(date_firstweek, periods=5)).dt.date(),
            }
        )
